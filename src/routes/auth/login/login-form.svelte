<script lang="ts">
  ...
  import { credentialSignIn, sending } from "$lib/stores/auth";
...
<form method="POST" use:enhance class="flex flex-col gap-2" action="?/login">
  <Form.Field {form} name="email">
    <Form.Control>
      {#snippet children({ props })}
        <Form.Label>Email</Form.Label>
        <Input {...props} bind:value={$formData.email} placeholder="john@example.com" required />
      {/snippet}
    </Form.Control>
    <Form.FieldErrors />
  </Form.Field>
